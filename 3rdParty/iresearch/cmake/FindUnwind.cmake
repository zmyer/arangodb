...
find_library(UNWIND_SHARED_LIBRARY_CORE
  NAMES unwind
  PATHS ${UNWIND_SEARCH_LIB_PATH} ${UNIX_DEFAULT_LIB} /usr/lib/x86_64-linux-gnu
  PATH_SUFFIXES ${UNWIND_ROOT_SUFFIX}
  NO_DEFAULT_PATH
)
find_library(UNWIND_SHARED_LIBRARY_LZMA
  NAMES lzma
  PATHS ${UNWIND_SEARCH_LIB_PATH} ${UNIX_DEFAULT_LIB}  "/usr/lib/x86_64-linux-gnu" "/lib/x86_64-linux-gnu"
  PATH_SUFFIXES ${UNWIND_ROOT_SUFFIX}
  NO_DEFAULT_PATH
)
...
if (Unwind_INCLUDE_DIR AND
    UNWIND_SHARED_LIBRARY_CORE AND UNWIND_SHARED_LIBRARY_PLATFORM AND
    UNWIND_STATIC_LIBRARY_CORE AND UNWIND_STATIC_LIBRARY_PLATFORM AND
    ( UNWIND_SHARED_LIBRARY_LZMA OR UNWIND_STATIC_LIBRARY_LZMA )
)
  ...
  #IMPORTED TARGETS
  add_library(unwind_static IMPORTED STATIC)
  set_target_properties(unwind_static PROPERTIES
      INTERFACE_INCLUDE_DIRECTORIES "${UNWIND_INCLUDE_DIR}"
      IMPORTED_LOCATION "${UNWIND_STATIC_LIBRARY_CORE}"
  )
  add_library(unwind_platform_static IMPORTED STATIC)
  set_target_properties(unwind_platform_static PROPERTIES
      INTERFACE_INCLUDE_DIRECTORIES "${UNWIND_INCLUDE_DIR}"
      IMPORTED_LOCATION "${UNWIND_STATIC_LIBRARY_PLATFORM}"
  )
  list(APPEND Unwind_STATIC unwind_static unwind_platform_static)
  ...
  if(UNWIND_SHARED_LIBRARY_LZMA)
    ...
    add_library(lzma_shared IMPORTED SHARED)
    set_target_properties(lzma_shared PROPERTIES
        INTERFACE_INCLUDE_DIRECTORIES "${UNWIND_INCLUDE_DIR}"
        IMPORTED_LOCATION "${UNWIND_SHARED_LIBRARY_LZMA}"
    )
    if(NOT UNWIND_STATIC_LIBRARY_LZMA)
        list(APPEND Unwind_STATIC lzma_shared)
    endif()
  endif()
  ...
  if(UNWIND_STATIC_LIBRARY_LZMA)
    ...
    add_library(lzma_static IMPORTED STATIC)
    set_target_properties(lzma_static PROPERTIES
        INTERFACE_INCLUDE_DIRECTORIES "${UNWIND_INCLUDE_DIR}"
        IMPORTED_LOCATION "${UNWIND_STATIC_LIBRARY_LZMA}"
    )
    list(APPEND Unwind_STATIC lzma_static)
  endif()
