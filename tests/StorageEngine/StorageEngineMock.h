...
// MockStorageEngine is typically a stack object that appears early
//  in a SECTION.  It should precede any TRI_vocbase_t objects, and must
//  outlive any TRI_vocbase_t objects.
struct MockStorageEngine : public StorageEngine {

  // Constructor saves current ENGINE global and destructor restores.
  //  This ASSUMES only one MockStorageEngine object, or that multiple
  //  destruct in reverse order of construction.  
  MockStorageEngine()
    : StorageEngine(nullptr, "MockEngine", "MockFeature", nullptr)
  {
    _old_global = EngineSelectorFeature::ENGINE;
    EngineSelectorFeature::ENGINE = this;
  }

  ~MockStorageEngine() {
    EngineSelectorFeature::ENGINE = _old_global;
  }
  
 protected:
  StorageEngine * _old_global;

  virtual bool supportsDfdb() const {return false;};

  virtual TransactionManager* createTransactionManager() {return nullptr;};
  virtual transaction::ContextData* createTransactionContextData() {return nullptr;};
  virtual TransactionState* createTransactionState(TRI_vocbase_t*, transaction::Options const&) {return nullptr;};
  virtual TransactionCollection* createTransactionCollection(TransactionState*, TRI_voc_cid_t, AccessMode::Type, int nestingLevel) {return nullptr;};
  ...
  virtual PhysicalCollection* createPhysicalCollection(LogicalCollection*, VPackSlice const&) {return nullptr;};

  // create storage-engine specific view
  virtual PhysicalView* createPhysicalView(LogicalView*, VPackSlice const&) {return nullptr;};
  ...
  virtual void getDatabases(arangodb::velocypack::Builder& result) {};
  ...
  virtual int getCollectionsAndIndexes(TRI_vocbase_t* vocbase, arangodb::velocypack::Builder& result,
                                       bool wasCleanShutdown, bool isUpgrade) {return 0;};
  
  virtual int getViews(TRI_vocbase_t* vocbase, arangodb::velocypack::Builder& result) {return 0;};
  
  // return the absolute path for the VERSION file of a database
  virtual std::string versionFilename(TRI_voc_tick_t id) const {return std::string();};

  // return the path for a database
  virtual std::string databasePath(TRI_vocbase_t const* vocbase) const {return std::string();};

  // return the path for a collection
  virtual std::string collectionPath(TRI_vocbase_t const* vocbase, TRI_voc_cid_t id) const {return std::string();};
  ...
  virtual void waitForSync(TRI_voc_tick_t tick) {};

  //// operations on databasea

  /// @brief opens a database
  virtual TRI_vocbase_t* openDatabase(arangodb::velocypack::Slice const& args, bool isUpgrade, int& status) {return nullptr;};
  ...
  virtual TRI_vocbase_t* createDatabase(TRI_voc_tick_t id, arangodb::velocypack::Slice const& args, int& status) {return nullptr;};
  ...
  virtual int writeCreateDatabaseMarker(TRI_voc_tick_t id, VPackSlice const& slice) {return 0;};
  ...
  virtual void prepareDropDatabase(TRI_vocbase_t* vocbase, bool useWriteMarker, int& status) {};
  ...
  virtual Result dropDatabase(TRI_vocbase_t*) {return Result();};

  /// @brief wait until a database directory disappears - not under lock in databaseFreature
  virtual void waitUntilDeletion(TRI_voc_tick_t id, bool force, int& status) {};
  ...
  virtual std::string createCollection(TRI_vocbase_t* vocbase, TRI_voc_cid_t id,
                                       arangodb::LogicalCollection const*) {return std::string();};
  ...
  virtual arangodb::Result persistCollection(
      ...
      arangodb::LogicalCollection const* collection) {return Result();};
  ...
  virtual arangodb::Result dropCollection(TRI_vocbase_t* vocbase, arangodb::LogicalCollection* collection) {return Result();};
  ...
  virtual void destroyCollection(TRI_vocbase_t* vocbase, arangodb::LogicalCollection* collection) {};
  ...
  virtual arangodb::Result renameCollection(
      ...
      std::string const& oldName) {return Result();};
  ...
  virtual arangodb::Result persistView(
      TRI_vocbase_t* vocbase, arangodb::LogicalView const*) {return Result();};
  ...
  // to "dropView" returns
  virtual arangodb::Result dropView(TRI_vocbase_t* vocbase, arangodb::LogicalView*) {return Result();};
  ...
  virtual void destroyView(TRI_vocbase_t* vocbase, arangodb::LogicalView*) {};
  ...
  virtual void unloadCollection(TRI_vocbase_t* vocbase, arangodb::LogicalCollection* collection) {};

  virtual void signalCleanup(TRI_vocbase_t* vocbase) {};

  virtual int shutdownDatabase(TRI_vocbase_t* vocbase) {return 0;};
  ...
  // replication
  virtual std::shared_ptr<arangodb::velocypack::Builder> getReplicationApplierConfiguration(TRI_vocbase_t*, int& status) {return std::shared_ptr<arangodb::velocypack::Builder>();};
  virtual int removeReplicationApplierConfiguration(TRI_vocbase_t* vocbase) {return 0;};
  virtual int saveReplicationApplierConfiguration(TRI_vocbase_t* vocbase, arangodb::velocypack::Slice slice, bool doSync) {return 0;}; 
  ...
  virtual int handleSyncKeys(arangodb::InitialSyncer& syncer,
                          ...
                          std::string& errorMsg) {return 0;};
  virtual Result createLoggerState(TRI_vocbase_t* vocbase, VPackBuilder& builder) {return Result();};
  virtual Result createTickRanges(VPackBuilder& builder) {return Result();};
  virtual Result firstTick(uint64_t& tick) {return Result();};
  virtual Result lastLogger(TRI_vocbase_t* vocbase
                           ,std::shared_ptr<transaction::Context>
                           ,uint64_t tickStart, uint64_t tickEnd
                           ,std::shared_ptr<VPackBuilder>& builderSPtr) {return Result();};

  virtual bool useRawDocumentPointers() {return false;};
